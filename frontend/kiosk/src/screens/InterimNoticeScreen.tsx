...
const InterimNoticeScreen: React.FC<InterimNoticeScreenProps> = ({
  ...
  return (
    ...
    >
      ...
      <div className="glass max-w-4xl w-full rounded-[48px] overflow-hidden relative z-10 flex flex-col border border-[var(--card-border)]" style={{ boxShadow: 'var(--card-shadow)' }}>
        
        {/* Header */}
        <div className="bg-[var(--warning)]/10 px-12 py-10 flex items-center gap-6 border-b border-[var(--card-border)]">
        ...
        <div className="px-12 py-12 space-y-10">
          <div className="space-y-4">
            <p className="text-2xl leading-relaxed font-medium" style={{ color: 'var(--text-main)', opacity: 0.9 }}>
            ...
            <p className="text-xl text-[var(--text-secondary)] leading-relaxed">
              Bạn vẫn có thể sạc ngay bây giờ, nhưng hệ thống sẽ <span className="font-bold">tự động dừng sạc</span> vào lúc <span className="font-bold">{stopTime}</span> (5 phút trước giờ hẹn) để đảm bảo quyền lợi cho khách đã đặt trước.
            </p>
          </div>

          <div className="grid grid-cols-2 gap-6">
            <div className="glass-pill p-6 flex items-center gap-4 border-[var(--card-border)]">
        ...
        {/* Actions */}
        <div className="px-12 py-10 bg-transparent flex gap-6 border-t border-[var(--card-border)]">
          <button 
            onClick={onCancel}
            className="flex-1 py-5 rounded-2xl border border-[var(--card-border)] hover:bg-[var(--card-border)] transition-all flex items-center justify-center gap-3 font-bold"
          >
            <X size={20} />
            HỦY BỎ
          </button>
